...
static struct descriptor
{
     ...
     struct descriptor *link; // 形成一个 在 htab 中 具有 相同 散列 值 的 块 描述 符 链表
     const void *ptr;         // 存储 块 的 地址 ，用于 其他 地方 分配, 相当于 key
     long size;               // 存储块 的 大小
...
void Mem_free(void *ptr, const char *file, int line)
{
     ...
     if (ptr)
     {
          ...
          // 将 bp 的 free  添加 到 freelist 中
          bp->free = freelist.free;
          freelist.free = bp;
     }
}
// Mem_alloc 返回 的 指针 总是 共用体 大小 的 倍数
union align
{
     int i;
     long l;
...
void *Mem_resize(void *ptr, long nbytes, const char *file, int line)
{
     ...
     //从 ptr 复制 nbytes < bp->size ? nbytes : bp->size  个字符到 newptr
     ...
     for (bp = freelist.free; bp; bp = bp->free)
     {
          ...
          if (bp->size > nbytes)
          {
               ...
               if ((bp = dalloc(ptr, nbytes, file, line)) != NULL)
               {
                    ...
                    bp->link = htab[h];
                    //该 空闲 存储块 底部 的 nbytes 个 字节 被 分割 出来，分割 出来 的 存储 块 的 描述 符 被 创建 ，初始化 并 添加 到 htab 中 
