...
struct T
{
    struct node
    {
        struct node *llink;
        *rlink;
        void *value; // ring 的 value
    } * head;        //  指向 一个 node 结构 的 双 向 链接表, head 指向 与 索引 0 相关 联 的 值 ，
    // 连续 的 值 存放 在 由 rlink 域 链接 的 node 里面 ，并且 每个 node 的 llink 域 都 指向 它的 前者
    int length;
};
...
void *Ring_get(T ring, int i)
{
...

    {
        int n;
        q = ring->head;
        //第一次循环 要 通过 rlink 指针 顺时针 旋转到 指定 节点
        if (i <= ring->length / 2)
        {
            for (n = i; n-- > 0;)
            {
